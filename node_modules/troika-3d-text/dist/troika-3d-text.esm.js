import { DataTexture, LuminanceFormat, LinearFilter, PlaneBufferGeometry, Vector3, InstancedBufferGeometry, Sphere, InstancedBufferAttribute, Vector2, Vector4, Matrix3, MeshBasicMaterial, DoubleSide, Matrix4, Mesh, Color, BoxBufferGeometry, Plane } from 'three';
import { defineWorkerModule, ThenableWorkerModule } from 'troika-worker-utils';
import { createDerivedMaterial } from 'troika-three-utils';
import { Instanceable3DFacade, createDerivedMaterial as createDerivedMaterial$1, ListFacade, Object3DFacade } from 'troika-3d';
...
function getMesh() {
  let material = createDerivedMaterial$1(
...
class SelectionManagerFacade extends ListFacade {
  constructor (parent, onSelectionChange) {
    ...
    const onDragStart = e => {
      ...
      if (textRenderInfo) {
        const localPoint = e.intersection.point.clone().applyMatrix4(tempMat4$1.getInverse(textMesh.matrixWorld));
    ...
    const onDrag = e => {
      ...
      if (e.ray && textRenderInfo) {
        ...
        const ray = e.ray.clone().applyMatrix4(tempMat4$1.getInverse(textMesh.matrixWorld));
        const localPoint = ray.intersectPlane(tempPlane.setComponents(0, 0, 1, 0), tempVec3$1);
        if (localPoint) {
          const caret = getCaretAtPoint(textRenderInfo, localPoint.x, localPoint.y);
          if (caret) {
...
export { GlyphsGeometry, Text3DFacade, TextMesh, configureTextBuilder, fontProcessorWorkerModule, preloadFont };
